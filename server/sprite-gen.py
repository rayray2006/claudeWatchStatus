...
lines: list[str] = [
    ...
    "/// streams (base64 encoded). Each sprite is decoded once per process into",
    "/// a CGImage and rendered via `Image(decorative:)` — a single GPU blit,",
    "/// not per-pixel Canvas fills. This keeps state transitions instant so the",
    "/// sync spinner can clear the moment the new state is committed.",
    ...
    "    }",
    "",
    "    /// Built once per process. Each CGImage wraps the raw RGBA byte",
    "    /// stream directly (no copy) and is ready for rendering.",
